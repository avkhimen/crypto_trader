...
class CryptoEnv():
    ...
    def select_random_window(self):
        ...
        self.start_index = np.random.randint(0, len(self.all_price_series) - 100)
        self.ser = np.array(self.all_price_series[self.start_index:self.start_index+self.window_size+self.lookup_interval+1])
    ...
    def step(self, state, action):
        ...
        self.step_order += 1
        self.ser = np.array(self.all_price_series[self.start_index+self.step_order:self.start_index+self.lookup_interval+self.step_order])
        self.ser_normalized = (np.array(self.ser)/self.ser[0]).tolist()
        next_state.extend(self.ser_normalized)
        done = False
